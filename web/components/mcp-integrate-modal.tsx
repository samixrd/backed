...
export function McpIntegrateModal({
  ...
  const grokPython = `# === xAI Grok (Grok-2 / Grok-beta) + BACKED Integration ===
from openai import OpenAI
import requests

# 1. Initialize Grok Client via xAI Endpoint
client = OpenAI(
    api_key="xai-your-api-key",
    base_url="https://api.x.ai/v1",
)

# 2. Ingest live BACKED Smart Money Intel across 718 Binance Futures
intel = requests.get("${PROD_URL}/api/intel?symbol=SOLUSDT").json()
long_pct = intel.get("topLongPct", 50)
taker = intel.get("takerRatio", 1.0)
regime = intel.get("regime", "Neutral")

# 3. Grok analyzes the institutional orderflow & formulates alpha execution
prompt = f"""
[BACKED x Binance Agent OS Intel]
Asset: SOLUSDT
Smart Money Regime: {regime}
Top Trader Dominance: {long_pct}% Long
Taker Aggression: {taker}x Buy/Sell Flow

As an autonomous trading agent, evaluate if whales are setting a trap or accumulating.
Formulate clear execution rationale and target.
"""

response = client.chat.completions.create(
    model="grok-2-latest",
    messages=[
        {"role": "system", "content": "You are a quantitative hedge fund agent powered by BACKED on Binance Agent OS."},
...
print(response.choices[0].message.content)`;
  ...
  // Nous Hermes 3 Python Snippet
  const hermesPython = `# === Nous Hermes-3 (Local / Ollama / OpenRouter) + BACKED MCP ===
import requests
import json

PROD_URL = "${PROD_URL}"

# 1. Pull live BACKED MCP tools (JSON-RPC 2.0)
mcp_res = requests.post(f"{PROD_URL}/api/mcp", json={
    "jsonrpc": "2.0",
    "id": 1,
    "method": "tools/list"
}).json()
backed_tools = mcp_res["result"]["tools"]

# 2. Convert MCP tools to Hermes / OpenAI function-calling format
hermes_tools = [
    {
        "type": "function",
        "function": {
    ...
    for t in backed_tools
]

# 3. Hermes-3 Agent Loop (via OpenRouter or local Ollama: 'hermes3:latest')
from openai import OpenAI
client = OpenAI(
    api_key="your-openrouter-or-ollama-key",
...
response = client.chat.completions.create(
    ...
    messages=[
        {"role": "system", "content": "You are an autonomous quant execution agent. Use BACKED tools to verify smart money sentiment before executing trades."},
        {"role": "user", "content": "Check smart money on BTCUSDT. If top traders are >= 65% long, execute a 5 USDT long order and return the BSC anchor."}
    ],
    tools=hermes_tools,
    tool_choice="auto"
)

# Hermes returns structured tool_calls:
print(json.dumps(response.choices[0].message.to_dict(), indent=2))`;

  // Universal Python MCP Client Snippet
  const pythonMcp = `# === Universal Python MCP Client (Claude / LangChain / CrewAI) ===
import asyncio
import httpx

PROD_MCP = "${PROD_URL}/api/mcp"

async def call_backed_tool(tool_name: str, arguments: dict):
    async with httpx.AsyncClient() as client:
        ...
        res = await client.post(PROD_MCP, json=payload, timeout=30.0)
        data = res.json()
        if "error" in data:
            raise RuntimeError(f"MCP Error: {data['error']}")
        return data["result"]["content"][0]["text"]

# Example: Get Intel & Execute Trade
async def main():
    # 1. Query smart money positioning
    intel = await call_backed_tool("backed_get_smart_money_intel", {"symbol": "BTCUSDT"})
    print("=== LIVE INTEL ===")
    print(intel)

    # 2. Execute intent-driven trade with BSC onchain anchor
    trade = await call_backed_tool("backed_execute_intent_trade", {
        "symbol": "BTCUSDT",
        "side": "BUY",
        "amountUsd": 5,
        "strategyReasoning": "Algorithmic execution based on smart accumulation"
    })
    print("\n=== TRADE ANCHORED RECEIPT ===")
    print(trade)
        ...
        <div className="p-6 space-y-5">
          ...
          {activeTab === "hermes" && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-xs text-foreground">
                  Native Function Calling for <strong>Nous Hermes 3</strong> (via OpenRouter / Ollama / vLLM):
                </p>
                <span className="text-[10px] text-accent font-semibold">Direct MCP Schema Binding</span>
              </div>
              <div className="relative rounded-lg border border-border bg-background p-3 max-h-[260px] overflow-y-auto">
                <pre className="text-[11px] text-muted overflow-x-auto select-all">{hermesPython}</pre>
                <button
                  onClick={() => handleCopy(hermesPython, "hermes")}
                  className="sticky top-0 float-right rounded border border-accent/30 bg-accent/10 px-2.5 py-1 text-[10px] font-bold text-accent hover:bg-accent hover:text-on-accent transition-colors"
                >
                  {copied === "hermes" ? "Copied ✓" : "Copy Code"}
                </button>
              </div>
            </div>
          )}
